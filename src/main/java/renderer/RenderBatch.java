...
//		shader = AssetPool.getShader("assets/shaders/default.glsl");
		...
		// Maximum batch size * 4 vertices per quad * vertex size (because each vertex will contain attributes such as x, y, z, color, etc.)
	...
	private void loadVertexProperties(int index) {
		...
		int texID = 0;
		// [0, tex, tex, tex, ...] 0 -> no texture
		// Map textures to their ID
		...
		if (isRotated) {
			transformMatrix.translate(
					sprite.gameObject.transform.position.x + sprite.gameObject.transform.offsetPivot.x,
					sprite.gameObject.transform.position.y + sprite.gameObject.transform.offsetPivot.y, 
					0
			);
	...
	private int[] generateIndices() {
		// Six indices per quad (because 3 per triangle)
	...
	private void loadElementIndicies(int[] elements, int i) {
